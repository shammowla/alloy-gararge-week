    ...
    context: {
      fields: {
        name: {
          value: "",
          errors: (str) =>
            checkIf(str).hasMinLength(1, "can't be blank").hasErrors(),
        },
        role: {
          value: "",
        },
        email: {
          value: "",
          errors: (str) => checkIf(str).isEmail().hasErrors(),
        },
        password: {
          value: "",
          errors: (str) =>
  ...
  },
  {
    ...
    guards: {
      isStep1Invalid: (ctx) => {
        ...
        const nameInvalid = name.errors(String(name.value)).length > 0;
        const emailInvalid = email.errors(String(email.value)).length > 0;
